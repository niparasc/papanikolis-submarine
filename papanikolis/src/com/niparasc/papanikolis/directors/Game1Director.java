...
public class Game1Director extends Director {
	...
	// Next surface point shall be generated at (xDistance + xVariation) from
	// the last surface point
	...
	// Random distances X and Y between surface points, used in multiplayer to
	// create the surfaces (recycled)
	...
	// Random Y distances for the obstacles, used in multiplayer to sync
	// obstacle positioning between peers (recycled)
	...
	// DEPRECATED: How much is velocity increased on Y axis when elevateSub is
	// true
	// protected static final float yUp = 0.08f;
	// DEPRECATED: How much is velocity decreased on Y axis when elevateSub is
	// false
	// protected static final float yDown = -0.05f;
	// The angle of the submarine when it is elevated (gives a feeling of speed
	// up)
	protected float pitch = 0;
	// The distance traveled by the submarine
	protected int distance = 0;
	// Distance at which the velocity is increased (keeps always the last
	// checkpoint)
	...
	public void direct() {
		...
		if (lastSurfacePoint.x - camera.position.x <= Papanikolis.VIEWPORT_WIDTH / 2) {
			...
			if (addObstacle) {
				...
				yObstacle = utils.random(nextSurfacePointBottom.y
						+ Obstacle.HEIGHT / 2, nextSurfacePointTop.y
						- Obstacle.HEIGHT / 2);
	...
	public void createSurfaceTop() {
		...
		vertices.add(new Vector2(Papanikolis.VIEWPORT_WIDTH, topMinY
				+ (topMaxY - topMinY) / 2));
	...
	public void createTunnelPoints() {
		...
		for (int i = 0; i < numberOfSurfacePoints; i++) {
			Vector2 nextSurfacePointBottom = new Vector2(lastPoint.x
					+ (xDistance + xVariation), utils.random(bottomMinY,
					bottomMaxY));
	...
	/*
	 * Creates and returns the mirror points of the bottom surface. These points
	 * are used to fill the surface with colored triangles.
	 */
	...
	/*
	 * Creates and returns the mirror points of the top surface. These points
	 * are used to fill the surface with colored triangles.
	 */
	...
	public void followSubmarine(Camera camera) {
		float x = submarine.getX()
				+ (Papanikolis.VIEWPORT_WIDTH / 2 - Papanikolis.VIEWPORT_WIDTH / 10)
				+ submarine.getWidth() / 2;
	...
	public void dispose() {
		...
		for (Obstacle obstacle : obstacles)
			obstacle.dispose();
		// Dispose obstacles in pool (out of game). Does this destroy the
		// physics bodies? Don't think so.
