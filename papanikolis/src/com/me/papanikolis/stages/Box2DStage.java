package com.me.papanikolis.stages;
...
public class Box2DStage extends Stage {
	...
	// Box2D world stepping parameters, Libgdx recommends 1/45f (or 1/300f) for mobile phones
...
//	}
	 ...
	 * === Free the physics ===
	 * 
	 * (solution implemented by Rotating Canvas too)
	 * This causes a subtle but visually unpleasant stuttering of the
	 * physics simulation on the screen known as temporal aliasing.
	 */
...
//	}
	...
	public void update(float dt) {
		...
		// We "smooth" positions and orientations using fixedTimestepAccumulatorRatio (alpha).
		smoothStates();
	}
...
/*	
	public void smoothStates() {
		float dt = fixedTimestepAccumulatorRatio * TIME_STEP;
		...
		while (it.hasNext()) {
			...
			Vector2 position = b.getLinearVelocity().mul(dt).add(b.getPosition());
			actor.setSmoothedPositionX(convertToWorld(position.x));
			actor.setSmoothedPositionY(convertToWorld(position.y));
			float angle = b.getAngle() + dt * b.getAngularVelocity();
...
*/
	...
	public void smoothStates() {
		...
		while (it.hasNext()) {
			...
			Box2DActor actor = (Box2DActor) b.getUserData();
			Vector2 previousPosition = new Vector2(convertToBox(actor.getPreviousPosition().x), convertToBox(actor.getPreviousPosition().y));
			Vector2 position = b.getPosition().mul(fixedTimestepAccumulatorRatio).add(previousPosition.mul(oneMinusRatio));
			actor.setSmoothedPositionX(convertToWorld(position.x));
			actor.setSmoothedPositionY(convertToWorld(position.y));
			float angle = fixedTimestepAccumulatorRatio * b.getAngle() + oneMinusRatio * actor.getPreviousAngle();
			actor.setSmoothedAngle(angle);
		}
	}
