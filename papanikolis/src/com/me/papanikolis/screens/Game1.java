	...
	public void render(float delta) {
		...
		if (!multiplayer) {
			...
			case READINESS_TEST:
				break;
			case SAILING:				
				// The director updates the actors (by giving instructions how to act, all game logic is here)
				director.direct();
				// Update physics world
				gameStage.update(delta);
//				gameStage.getDebugRenderer().render(gameStage.getWorld(), gameStage.getCamera().combined);
				distanceLabel.setText(" Distance: " + ((Game1Director)director).getDistance());
				break;
			case CRASHED:
				// Reposition the camera to the submarine's position.
				// For some reason Libgdx positions the camera to its initial position (at game start)
				// when pausing and resuming the game activity (home button on phone).
	...
	public void show() {
        ...
        menuButton.addListener(new ChangeListener() {
			...
			public void changed(ChangeEvent event, Actor actor) {
				if (bluetoothInterface != null && bluetoothInterface.isConnected()) {
		...
		bestDistanceLabel = new Label("Best: " + game.getPreferencesManager().getInteger("bestDistance", 0), getSkin());
		getTable().add(bestDistanceLabel).expandX().uniform().align(Align.center);
		
		getTable().row();
		
		infoLabel = new Label("At your command chief!\n\nTouch screen to fire the engines!", getSkin());
