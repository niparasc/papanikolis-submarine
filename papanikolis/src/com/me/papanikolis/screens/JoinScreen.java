...
public class JoinScreen extends AbstractScreen {
	...
	public void show() {
		...
		getTable().add(devicesListScrollPane).colspan(3).align(Align.center).spaceBottom(10);
		getTable().row();

        backButton = new TextButton("Back", getSkin());
        backButton.setVisible(false);
        getTable().add(backButton).size(150, 40).expand().bottom().spaceBottom(5).spaceRight(5);
        backButton.addListener(new ChangeListener() {
			@Override
			public void changed(ChangeEvent event, Actor actor) {
				// If connection is in progress, do nothing
				if (bluetoothInterface.isConnecting() || bluetoothInterface.isDiscovering())
					return;
        ...
        getTable().add(scanButton).size(150, 40).expand().bottom().spaceBottom(5).spaceLeft(5).spaceRight(5);
        scanButton.addListener(new ChangeListener() {
			@Override
			public void changed(ChangeEvent event, Actor actor) {
				if (!bluetoothInterface.isDiscovering() && !bluetoothInterface.isConnecting() && !bluetoothInterface.isConnected()) {
        ...
        getTable().add(connectButton).size(150, 40).expand().bottom().spaceBottom(5).spaceLeft(5);
        connectButton.addListener(new ChangeListener() {
			@Override
			public void changed(ChangeEvent event, Actor actor) {
				if (!bluetoothInterface.isDiscovering() && !bluetoothInterface.isConnecting()) {
					// To get the MAC from the string subtract the length of the MAC from the end of the string
					// and get the substring beginning at that index.
					String selectedDevice = devicesList.getSelection();
					
					if (selectedDevice != null) {
						String mac = selectedDevice.substring(selectedDevice.length() - 17);
		...
		if (bluetoothInterface.isBluetoothSupported()) {
			...
			if (!bluetoothInterface.isBluetoothEnabled()) {
				...
				Gdx.app.log(LOG, "isBluetoothEnabled = " + bluetoothInterface.isBluetoothEnabled());
				infoLabel.setText("Bluetooth is enabled! Select device to connect!");
