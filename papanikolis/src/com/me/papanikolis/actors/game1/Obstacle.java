...
public class Obstacle extends Box2DActor {
	...
	public void createBody(float x, float y) {
		...
		// Subtract the radius from the polygon shape so that there is no gap when bodies collide!
		// See box2d manual section 4.4. Shrinks the skin.
		ps.setAsBox(gameStage.convertToBox(getWidth() / 2) - ps.getRadius(), gameStage.convertToBox(getHeight() / 2) - ps.getRadius());
	...
	public void repositionBody(float x, float y) {
		body.setTransform(gameStage.convertToBox(x), gameStage.convertToBox(y), 0);
	}
