...
import com.me.papanikolis.actors.game1.PeerSubmarine;
...
import com.me.papanikolis.multiplayer.TransmissionPackagePool;
...
public abstract class MultiplayerDirector extends Game1Director {
	...
	public MultiplayerDirector(Papanikolis game, Box2DStage gameStage) {
		...
		createPeerSubmarine(submarine.getX() - submarine.getWidth() / 2, submarine.getY());
	}
	...
	public void direct() {
		...
		if (lastSurfacePoint.x - camera.position.x <= Papanikolis.VIEWPORT_WIDTH / 2) {
			...
			if (addObstacle) {
				...
				yObstacle = nextSurfacePointBottom.y + randomYObstacleDistance[k];
				k++;
...
//			submarineBody.applyLinearImpulse(new Vector2(0, 0), submarineBody.getLocalCenter());
//			v.y += yUp;
			pitch = 0.1f;
		} else {
//			v.y += yDown;
	...
	/*
	 * Creates the random point distances that will be used (recycled) to
	 * create the surfaces and position the obstacles.
	 */
	public void createRandomPointDistances() {
		...
		for (int i = 0; i < randomYObstacleDistance.length; i++) {
			randomYObstacleDistance[i] = utils.random(Obstacle.HEIGHT / 2, topMinY - bottomMaxY);
		}
	}
	...
	public void createTunnelPoints() {
		...
		for (int i = 0; i < numberOfSurfacePoints; i++) {
			Vector2 nextSurfacePointBottom = new Vector2(lastPoint.x + randomPointXDistance[k], randomPointYDistance[k]);
			k++;
